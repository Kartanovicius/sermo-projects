...
// Material-ui
import { AlertColor, Avatar, Box, Button, Card, Grid, Icon, Paper, Skeleton, TextField, Typography } from '@mui/material'
// npm packages
...
export default function UserInformation({setAlerts}: Props) {
  ...
  // Updated user fields 
  let alertMessage: {message: string, variant: AlertColor}[] = []
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <div>
                <TextField id='firstnameInput' label='First Name' variant='filled' defaultValue={userFirst} inputRef={userFirstRef}/>
                <TextField id='lastnameInput' label='Last Name' variant='filled' defaultValue={userLast} inputRef={userLastRef}/>
              </div>
        ...
        <Paper sx={{ width: 'inherit', textAlign: 'end', display: 'inline-flex', gap: 2, justifyContent: 'end', p: 1 }}>
          <Button variant='outlined' onClick={() => resetUserInfo()}>Reset</Button>
          <Button variant='contained' onClick={() => saveUserInfoHandler()}>Save</Button>
        </Paper>
      </Card>
      }
    </Grid>
